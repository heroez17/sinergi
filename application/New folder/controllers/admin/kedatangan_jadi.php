...
class Kedatangan_jadi extends CI_Controller {
	...
	{   
		...
		$isi['log'] = "";
		$isi['home_nav'] = "";
		$isi['home_nav1'] = "DATA KEDATANGAN BAHAN JADI";
		...
		$isi['data'] = $this->db->query('SELECT * FROM tab_pembelian_jadi where stts="KIRIM"');
		$isi['kontent'] = "admin/kedatangan_jadi/kontent";
	...
	public function proses($id){
		...
		$isi['log_sub'] = "";
		$isi['header'] = "admin/header";
		$isi['cari'] = $this->db->query('SELECT * FROM tab_pembelian_jadi where stts="KIRIM" and id_pembelian="'.$id.'"');
$isi['cari_table']=$this->db->query('select * from tab_pembelian_detail_jadi where id_pembelian="'.$id.'" and stts!="NONAKTIF"');
$isi['loop']=$this->db->query('select * from tab_pembelian_detail_jadi where id_pembelian="'.$id.'" and stts!="NONAKTIF"')->num_rows();

		$isi['kontent'] = "admin/kedatangan_jadi/kontent_table";
		$this->load->view('admin/tampilan_home',$isi);
	...
	public function simpan(){
		 ...
		 for($i=1;$i<=$loop;$i++){
			...
			$this->db->query("UPDATE ibenk_pantri.tab_pembelian_jadi SET stts = 'SELESAI' WHERE id_pembelian = '$id_pembelian' ;");
			$this->db->query("UPDATE ibenk_pantri.tab_pembelian_detail_jadi 
												SET stts = 'OKE'  WHERE
												id_pembelian_detail = '$id_pembelian_detail' AND id_pembelian = '$id_pembelian' ;");
			$this->db->query("INSERT INTO ibenk_pantri.tab_stock_jadi VALUES
										('', 
										'$id_pembelian', 
										'$id_pembelian_detail', 
										'$userinput', 
			...
			redirect("admin/kedatangan_jadi");
		}
