...
<tbody>
	...
	$number=1;
	$sum=0;
	 $cari=$this->db->query("SELECT tab_pengeluaran_jadi.id_po
    , SUM(tab_pengeluaran_jadi.qty) AS qty
    , tab_jadi.nama AS dasar
    , tab_satuan.nama AS satuan
FROM
    ibenk_pantri.tab_pengeluaran_jadi
    INNER JOIN ibenk_pantri.tab_jadi 
        ON (tab_pengeluaran_jadi.id = tab_jadi.id_jadi)
    INNER JOIN ibenk_pantri.tab_satuan 
        ON (tab_jadi.satuan = tab_satuan.id_satuan) WHERE tab_pengeluaran_jadi.id_po='$id_po' AND tab_pengeluaran_jadi.id LIKE 'JD%' and tab_pengeluaran_jadi.qty !='0' GROUP BY tab_pengeluaran_jadi.id;");
