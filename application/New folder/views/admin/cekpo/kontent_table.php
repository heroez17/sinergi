...
function selesai(dasar,idpo,idpodet){
		$.ajax({
                        type: "POST",
                        url : "<?php echo base_url('index.php/admin/cekpo/selesai')?>",
                        data: 'dasar='+dasar+'&idpo='+idpo+'&idpodet='+idpodet, 
						beforeSend : function(){
							
							$('#add_page').html('<h1  align="center"><i class="fa fa-spin fa-refresh"></i></h1>');},
...
<?php } ?>
  ...
  if($carr==""){?><div class="col-xs-12">
        ...
        <input type="hidden" name="id_po" value="<?php echo $idpo;?>" />
          <button type="submit" class="btn btn-success pull-right"><i class="glyphicon glyphicon-ok"></i> Kirim Ke Warehouse 
          </button>
          </form>
          <?php } ?>
        </div>
